...
NAME_AND_REPLY_COLORS = [
    ...
    # Ряд 2: 7 двухцветных градиентов (7..13)
    "7 - 🔴/🟠 Красный + Оранжевый",
    "8 - 🟠/🟡 Оранжевый + Желтый",
    "9 - 🟣/🌸 Фиолетовый + Розовый",
    "10 - 🟢/🌊 Зеленый + Бирюзовый",
    "11 - 🌊/🔵 Бирюзовый + Синий",
    "12 - 🔵/🟣 Синий + Фиолетовый",
    "13 - 🌸/🔴 Розовый + Красный",
    ...
    "15 - 🟠/🟡/🟢 Оранжевый + Желтый + Зеленый",
    "16 - 🟣/🌸/🔴 Фиолетовый + Розовый + Красный",
    "17 - 🟢/🌊/🔵 Зеленый + Бирюзовый + Синий",
...
PEER_COLOR_RGB_MAP = {
    ...
    8: -27392,     # 8: 🟠 Оранжевый градиент
    9: -5745161,   # 9: 🟣 Фиолетовый градиент
    10: -5745161,  # 10: 🟣 Фиолетовый градиент (обложка)
    ...
    16: -5745161,  # 16: 🟣/🌸/🔴 Фиолетовый узор
    17: -11870592, # 17: 🟢/🌊/🔵 Зеленый узор
    18: -14494738, # 18: 🌊/🔵/🟣 Бирюзовый узор
    19: -10443270, # 19: 🔵/🟣/🌸 Синий узор
    20: -757066,   # 20: 🌸/🔴/🟠 Розовый узор
}
...
class Plugin(BasePlugin):
    ...
    def _get_vibrant_color_for_id(self, color_id: int, is_profile: bool = False) -> int:
        ...
        if is_profile:
            ...
            if c == 0:
                return -306606
        if c in PEER_COLOR_RGB_MAP:
            return PEER_COLOR_RGB_MAP[c]
        return PEER_COLOR_RGB_MAP.get(c % 7, -10443270)
                        ...
                        try:
                            ...
                            if res is None or res == 0:
                                ...
                                if this_pc:
                                    ...
                                    param.setResult(plugin_self._get_vibrant_color_for_id(c_id))
                        except Exception:
                            pass
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                ...
                if resp.status == 200:
                    ...
                    server_ts = data.get("timestamp", int(time.time()))
                    ...
                    if show_bulletin:
                        bulletins.show_success(f"База обновлена! Загружено профилей: {len(profiles)}")
                    return
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=6) as resp:
                ...
                if resp.status == 200:
                    ...
                    new_ts = data.get("timestamp", int(time.time()))
    ...
    def create_settings(self) -> List[Any]:
        ...
        items = [
            ...
            Switch(
                key="enable_custom_badges",
                text="Отображать кастомные бейджи",
                default=True,
                subtext="Показывать кастомные бейджи рядом с именами в чатах и профилях",
                icon="msg_verified",
                on_change=lambda val: run_on_ui_thread(self._apply_all_to_all_accounts),
            ),
        ]
        ...
        for acc_info in active_accs:
            ...
            items.extend([
                ...
                Input(
                    key=f"slot_{acc_idx}_custom_badge",
                    text=f"Кастомный бейдж [{name}]",
                    default=cur_badge,
                    subtext="Текст плашки рядом с ником (например: VIP, Dev, Admin, ⚡ Pro)",
                    icon="msg_verified",
                    on_change=lambda val, a=acc_idx: (self._set_slot_val(a, "custom_badge", val), run_on_ui_thread(self._apply_all_to_all_accounts)),
                ),
                Selector(
