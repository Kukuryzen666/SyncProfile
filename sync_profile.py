...
class SyncProfilePlugin(BasePlugin):
    ...
    def _ensure_ayugram_premium(self):
        ...
        try:
            ...
            pass
        try:
            from com.kotogram.messenger import KotoConfig
            if hasattr(KotoConfig, 'localPremium'):
                if self.get_setting('enable_local_premium', True):
                    KotoConfig.localPremium = True
        except Exception:
            pass
        try:
            from org.telegram.messenger import SharedConfig
            if hasattr(SharedConfig, 'autoplayGifs'):
                SharedConfig.autoplayGifs = True
            if hasattr(SharedConfig, 'autoplayVideo'):
                SharedConfig.autoplayVideo = True
            if hasattr(SharedConfig, 'devicePerformanceClass'):
                SharedConfig.devicePerformanceClass = 2
    ...
    def _on_local_premium_toggle(self, enabled: bool):
        ...
        try:
            from com.kotogram.messenger import KotoConfig
            if hasattr(KotoConfig, 'localPremium'):
                KotoConfig.localPremium = bool(enabled)
        except Exception:
            pass
        try:
            from org.telegram.messenger import SharedConfig
