...
class SyncProfilePlugin(BasePlugin):
    ...
    def _import_settings_from_current_account(self, acc_idx: int, show_bulletin: bool=True):
        try:
            ...
            if not curr_user:
                if show_bulletin:
                    run_on_ui_thread(lambda : BulletinHelper.show_error('Не удалось получить данные пользователя.'))
                return
            nc = 0
            nbg = ''
            ...
            self._set_slot_val(acc_idx, 'name_bg_emoji_id', nbg)
            self._set_slot_val(acc_idx, 'profile_color', prc)
            self._set_slot_val(acc_idx, 'profile_bg_emoji_id', pbg)
            ...
            if show_bulletin:
                msg_ok = f'✨ Настройки успешно скопированы из Telegram!\nЦвет: {nc}, Обложка: {prc}, Эмодзи: {em_id or "нет"}'
                run_on_ui_thread(lambda : BulletinHelper.show_success(msg_ok))
        except Exception as e:
            log(f'SyncProfile: Ошибка ручного импорта: {e}')
            if show_bulletin:
                run_on_ui_thread(lambda : BulletinHelper.show_error(f'Ошибка копирования: {e}'))
    ...
    def _ensure_ayugram_premium(self):
        try:
            ...
            try:
                ...
                if hasattr(ExteraConfig, 'saveConfig'):
            ...
            try:
                ...
                for acc in range(4):
                    try:
                        u_cfg = UserConfig.getInstance(acc)
                        if u_cfg and u_cfg.isClientActivated():
                            if hasattr(u_cfg, 'isPremium'):
    ...
    def _toggle_local_premium(self, enabled: bool):
        try:
            try:
                from com.radolyn.ayugram import AyuConfig
                if hasattr(AyuConfig, 'localPremium'):
        ...
        try:
            ...
            for acc in range(max_accs):
                try:
                    ...
                    if u_cfg and u_cfg.isClientActivated():
                        curr_u = u_cfg.getCurrentUser()
                        uid = int(u_cfg.getClientUserId() or (getattr(curr_u, 'id', 0) if curr_u else 0))
    ...
    def _get_active_accounts_data(self) -> List[Dict[str, Any]]:
        ...
        if not accounts:
            try:
                from org.telegram.messenger import UserConfig
                curr_user = UserConfig.getInstance(0).getCurrentUser()
                uid = int(getattr(curr_user, 'id', 0) or 0) if curr_user else 0
                if uid > 0:
                    fn = str(getattr(curr_user, 'first_name', '') or '')
                    ln = str(getattr(curr_user, 'last_name', '') or '')
                    uname = str(getattr(curr_user, 'username', '') or '')
    ...
    def _get_slot_val(self, acc_idx: int, key: str, default: Any=None) -> Any:
        return self.get_setting(f'slot_{acc_idx}_{key}', default)

    def _set_slot_val(self, acc_idx: int, key: str, value: Any):
        self.set_setting(f'slot_{acc_idx}_{key}', value, reload_settings=False)
    ...
    def _resolve_emoji_id_for_user(self, user_or_uid: Any) -> int:
        ...
        if uid > 0:
            ...
            if prof and prof.get('name_bg_emoji_id'):
                return int(prof['name_bg_emoji_id'])
            try:
                from org.telegram.messenger import UserConfig
                max_accs = getattr(UserConfig, 'MAX_ACCOUNT_COUNT', 4)
        ...
        if hasattr(user_or_uid, 'color'):
            c = getattr(user_or_uid, 'color', None)
            if c and hasattr(c, 'background_emoji_id') and c.background_emoji_id:
                return int(c.background_emoji_id)
        return 0

    def _extract_msg_sender_uid(self, msg_obj: Any) -> int:
        if not msg_obj:
            return 0
        try:
    ...
    def _extract_reply_uid(self, msg: Any) -> int:
        ...
        try:
            r_hdr = getattr(mo, 'reply_to', None) if mo else getattr(msg, 'reply_to', None)
            if r_hdr:
                r_from = getattr(r_hdr, 'reply_from', None)
                if r_from:
                    rf_peer = getattr(r_from, 'from_id', None)
                    uid = self._extract_peer_uid(rf_peer)
                    if uid > 0:
                        return uid
                r_peer = getattr(r_hdr, 'reply_to_peer_id', None)
                if r_peer:
                    uid = self._extract_peer_uid(r_peer)
                    if uid > 0:
                        return uid
        except Exception:
            pass
        try:
            rmo = getattr(msg, 'replyMessageObject', None)
            if rmo:
                from_peer = getattr(rmo, 'getFromPeer', lambda : None)()
                if from_peer:
                    uid = self._extract_peer_uid(from_peer)
                    if uid > 0:
                        return uid
                rmo_owner = getattr(rmo, 'messageOwner', None)
                if rmo_owner:
                    f_id = getattr(rmo_owner, 'from_id', None)
                    uid = self._extract_peer_uid(f_id)
                    if uid > 0:
                        return uid
                r_dialog_id = int(getattr(rmo, 'getDialogId', lambda : 0)() or 0)
                if r_dialog_id > 0:
                    return r_dialog_id
        except Exception:
            pass
        try:
            fwd = getattr(mo, 'fwd_from', None) if mo else getattr(msg, 'fwd_from', None)
            if fwd:
                fwd_from = getattr(fwd, 'from_id', None)
                uid = self._extract_peer_uid(fwd_from)
                if uid > 0:
                    return uid
        except Exception:
            pass
        ...
        try:
            ...
            if ThemeClass:
                ...
                for m in ThemeClass.getDeclaredMethods():
                    try:
                        if m.getName() == 'getColor' and len(m.getParameterTypes()) == 1:
            ...
            if ChatMessageCellClass:
                ...
                def _apply_cell_reply_color(cell, msg, is_draw_phase: bool=False):
                    if not cell or not msg:
                        return
                    reply_uid = plugin_self._extract_reply_uid(msg)
                    if reply_uid <= 0:
                        return
                    prof = plugin_self.get_cached_profile(reply_uid)
                    if not prof:
                        return
                    target_color = int(prof.get('name_color', 0) or 0)
                    vibrant_rgb = plugin_self._get_vibrant_color_for_id(target_color)
                    if vibrant_rgb == 0:
                        return
                    ...
                    for field_name in ('replyNamePaint', 'replyLinePaint', 'quotePaint', 'replyLine'):
                        try:
                            ...
                            if f_paint:
                                if hasattr(f_paint, 'setColor'):
                                    f_paint.setColor(vibrant_rgb)
                                if hasattr(f_paint, 'color'):
                                    f_paint.color = vibrant_rgb
                        except Exception:
                            pass
                    for getter_name in ('getReplyNamePaint', 'getReplyLinePaint', 'getQuotePaint'):
                        try:
                            gp = getattr(cell, getter_name, lambda : None)()
                            if gp and hasattr(gp, 'setColor'):
                                gp.setColor(vibrant_rgb)
                        except Exception:
                            pass
                    if ThemeClass:
                    ...
                    def after_hooked_method(self, param):
                        try:
                            ...
                            if args and len(args) > 0 and args[0] is not None:
                                ...
                                if uid > 0:
                                    if uid in plugin_self._get_my_active_uids():
                                        if plugin_self.get_setting('enable_local_premium', True):
                                            param.setResult(True)
                                    else:
                        ...
                        try:
                            ...
                            if args and len(args) > 0 and hasattr(args[0], 'id'):
                                ...
                                if uid > 0:
                                    prof = plugin_self.get_cached_profile(uid)
                                    if prof:
                                        param.setResult(int(prof.get('name_color', 0) or 0))
                        except Exception:
                            pass
                    ...
                    def after_hooked_method(self, param):
                        try:
                            ...
                            if args and len(args) > 0 and hasattr(args[0], 'id'):
                                ...
                                if uid > 0:
                                    prof = plugin_self.get_cached_profile(uid)
                                    if prof and int(prof.get('profile_color', 0) or 0) > 0:
                                        param.setResult(int(prof['profile_color']))
                        except Exception:
                            pass
                    ...
                    def after_hooked_method(self, param):
                        try:
                            ...
                            if args and len(args) > 0 and args[0] is not None:
                                ...
                                if uid > 0:
                                    prof = plugin_self.get_cached_profile(uid)
                                    if prof and (int(prof.get('name_color', 0) or 0) > 0 or prof.get('name_bg_emoji_id')):
                                        param.setResult(True)
                                    elif hasattr(user, 'color') and user.color is not None:
                                        param.setResult(True)
                        except Exception:
                            pass
                            ...
                            if args:
                                ...
                                if user is not None:
                                    ...
                                    if uid > 0:
                                        prof = plugin_self.get_cached_profile(uid)
                                        if prof:
                                            c_id = int(prof.get('name_color', 0) or 0)
                ...
                class MsgObjGetColorIdHook(MethodHook):
                    ...
                    def after_hooked_method(self, param):
                        try:
                            ...
                            if msg_obj:
                                uid = plugin_self._extract_msg_sender_uid(msg_obj)
                                if uid > 0:
                                    prof = plugin_self.get_cached_profile(uid)
                                    if prof:
                                        param.setResult(int(prof.get('name_color', 0) or 0))
                    ...
                    def after_hooked_method(self, param):
                        try:
                            ...
                            if msg_obj:
                                ...
                                if r_uid > 0:
                                    prof = plugin_self.get_cached_profile(r_uid)
                                    if prof:
                                        param.setResult(int(prof.get('name_color', 0) or 0))
                        except Exception:
                            pass
                        ...
                        try:
                            ...
                            if msg_obj:
                                ...
                                if uid > 0:
                                    prof = plugin_self.get_cached_profile(uid)
                                    if prof:
                                        nc = int(prof.get('name_color', 0) or 0)
                ...
                for m in MessageObjectClass.getDeclaredMethods():
                    try:
                        ...
                        if m_name == 'getColorId' and p_count == 0:
                            ...
                            if un:
                                self._xposed_unhooks.append(un)
                        elif m_name == 'getEmojiId' and p_count == 0:
                            m.setAccessible(True)
                            un = self.hook_method(m, MsgObjGetEmojiIdHook())
                            if un:
                                self._xposed_unhooks.append(un)
                        elif m_name == 'getReplyEmojiId' and p_count == 0:
                            m.setAccessible(True)
                            un = self.hook_method(m, MsgObjGetReplyEmojiIdHook())
                            if un:
                                self._xposed_unhooks.append(un)
                        elif m_name == 'hasEmoji' and p_count == 0:
                            m.setAccessible(True)
                            un = self.hook_method(m, MsgObjHasEmojiHook())
                            if un:
                                self._xposed_unhooks.append(un)
                        elif m_name == 'hasReplyEmoji' and p_count == 0:
                            m.setAccessible(True)
    ...
    def _queue_fetch_users_bulk(self, uids: List[int]):
        if not uids or not self.get_setting('enable_sync', True):
            return
        ...
        with self._sync_lock:
            ...
            if len(self._pending_user_ids) >= 15:
                ...
                self._batch_timer = threading.Timer(0.2, self._execute_batch_fetch)
                        ...
                        def on_batch_loaded():
                            try:
                                ...
                                for acc in range(4):
                                    ...
                                    if mc and n_c:
                                        for u_id in ids:
                                            if u_id in self._profiles_cache:
                                                u = mc.getUser(u_id)
                                                if u:
                                                ...
                                                if fu:
                                                    ...
                                                    mc.putUserFull(fu)
                                        for notif_name in ('replaceMessagesObjects', 'replaceMessagesText', 'didUpdateMessagesViews', 'reloadDialogPhotos', 'peerColorsDidLoad', 'themeAccentListUpdated'):
                                            try:
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=7) as resp:
                if resp.status == 200:
                    ...
                    if show_bulletin:
                        bulletins.show_success(f'Обновлено профилей: {len(new_cache)}')
                    return True
        except Exception as e:
            logger.error(f'Ошибка дельта-синхронизации: {e}')
            if show_bulletin:
    ...
    def _patch_user_tl_object(self, u: Any) -> bool:
        ...
        try:
            ...
            nbg = int(nbg_val) if nbg_val and str(nbg_val).isdigit() and int(nbg_val) != 0 else 0
            prc = int(profile.get('profile_color', 0) or 0)
            if prc < 0:
                prc = 0
            pbg_val = profile.get('profile_bg_emoji_id', '')
            pbg = int(pbg_val) if pbg_val and str(pbg_val).isdigit() and int(pbg_val) != 0 else 0
            em_id_val = profile.get('emoji_status_id', '')
            em_id = int(em_id_val) if em_id_val and str(em_id_val).isdigit() and int(em_id_val) != 0 else 0
            ...
            log(f'SyncProfile: _patch_user_tl_object error: {e}')
            return False

    def _patch_full_user_tl_object(self, fu: Any, target_uid: int=0) -> bool:
        if not fu:
            return False
        uid = target_uid
        if uid == 0:
            uid = int(getattr(fu, 'id', 0) or 0)
        if uid == 0 and hasattr(fu, 'user') and fu.user:
            uid = int(getattr(fu.user, 'id', 0) or 0)
        if uid == 0:
            return False
        profile = self.get_cached_profile(uid)
        if not profile:
        ...
        try:
            ...
            em_id_val = profile.get('emoji_status_id', '')
            em_id = int(em_id_val) if em_id_val and str(em_id_val).isdigit() and int(em_id_val) != 0 else 0
            if hasattr(fu, 'profile_color') and (prc > 0 or pbg > 0):
                fu.profile_color = _build_peer_color(prc, pbg)
                fu.flags2 = int(getattr(fu, 'flags2', 0) | 512)
    ...
    def grab_settings_from_local_premium(self, target_acc_idx: int=-1, show_bulletin: bool=True):
        ...
        if not extracted_data:
            try:
                ExteraConfigClass = find_class('org.telegram.messenger.ExteraConfig')
                if ExteraConfigClass:
                    for f in ExteraConfigClass.getDeclaredFields():
                        try:
                            f.setAccessible(True)
                            fname = f.getName().lower()
                            fval = f.get(None)
        ...
        try:
            ...
            if u_cfg:
                curr_user = u_cfg.getCurrentUser()
                if curr_user:
                    c_val = getattr(curr_user.color, 'color', None) if hasattr(curr_user, 'color') and curr_user.color else None
                    c_em = getattr(curr_user.color, 'background_emoji_id', None) if hasattr(curr_user, 'color') and curr_user.color else None
                    if c_val is not None:
                        extracted_data['name_color'] = int(c_val)
                    if c_em:
                        extracted_data['name_bg_emoji_id'] = str(c_em)
                    pc_val = getattr(curr_user.profile_color, 'color', None) if hasattr(curr_user, 'profile_color') and curr_user.profile_color else None
                    pc_em = getattr(curr_user.profile_color, 'background_emoji_id', None) if hasattr(curr_user, 'profile_color') and curr_user.profile_color else None
                    if pc_val is not None:
                        extracted_data['profile_color'] = int(pc_val)
                    if pc_em:
                        extracted_data['profile_bg_emoji_id'] = str(pc_em)
                    st_em = getattr(curr_user.emoji_status, 'document_id', None) if hasattr(curr_user, 'emoji_status') and curr_user.emoji_status else None
                    if st_em:
                        extracted_data['emoji_status_id'] = str(st_em)
                mc = MessagesController.getInstance(target_acc_idx)
                if mc and curr_user:
                    uf = mc.getUserFull(getattr(curr_user, 'id', 0))
                    if uf and hasattr(uf, 'profile_color') and (uf.profile_color is not None):
                        ...
                        if pc_em:
                            extracted_data['profile_bg_emoji_id'] = str(pc_em)
                    if uf and hasattr(uf, 'color') and (uf.color is not None):
                        c_val = getattr(uf.color, 'color', None)
                        c_em = getattr(uf.color, 'background_emoji_id', None)
                        if c_val is not None:
                            extracted_data['name_color'] = int(c_val)
                        if c_em:
                            extracted_data['name_bg_emoji_id'] = str(c_em)
                    if uf and hasattr(uf, 'emoji_status') and (uf.emoji_status is not None):
                        doc_id = getattr(uf.emoji_status, 'document_id', 0)
                        if doc_id:
        ...
        if extracted_data:
            ...
            self.set_setting(f'slot_{target_acc_idx}_configured', True, reload_settings=False)
            self._save_local_profiles_cache(force=True)
            self._apply_all_to_all_accounts()
            if show_bulletin:
                c_n = extracted_data.get('name_color', 0)
                c_p = extracted_data.get('profile_color', 0)
                em_st = extracted_data.get('emoji_status_id', '')
                msg_ok = f'✨ Настройки локального Premium загружены для {name}!\nИмя/Реплаи: #{c_n} | Обложка: #{c_p} | Статус: {em_st or "нет"}\nНажмите "🚀 Опубликовать", чтобы отправить на сервер!'
                run_on_ui_thread(lambda : BulletinHelper.show_success(msg_ok))
        elif show_bulletin:
            msg_info = f'В локальном Premium для {name} не найдено настроенных цветов.\nУстановите их вручную в меню плагина.'
            run_on_ui_thread(lambda : BulletinHelper.show_info(msg_info))

    def push_specific_account(self, acc_idx: int, show_ui_bulletin: bool=True) -> bool:
        active_accs = self._get_active_accounts_data()
        matched = [a for a in active_accs if a['acc_idx'] == acc_idx]
        if not matched:
            if show_ui_bulletin:
                bulletins.show_error(f'Аккаунт слот {acc_idx + 1} не найден или не активен.')
            return False
        acc_info = matched[0]
        ...
        try:
            ...
            with urllib.request.urlopen(req, timeout=8) as resp:
                if resp.status == 200:
                    ...
                    bulletins.show_error(f'Ошибка сервера: статус {resp.status}')
        except Exception as e:
            logger.error(f'Ошибка публикации {name}: {e}')
            if show_ui_bulletin:
                bulletins.show_error(f'Ошибка отправки: {e}')
        return False
    ...
    def push_all_accounts(self, show_ui_bulletin: bool=True):
        ...
        for acc_info in active_accs:
            ...
            try:
                ...
                logger.error(f"Ошибка отправки профиля ID {uid}: {e}")
        self._save_local_profiles_cache(force=True)
        self._apply_all_to_all_accounts()
        if show_ui_bulletin:
            if success_count > 0:
                bulletins.show_success(f'Успешно отправлено {success_count} из {len(active_accs)} профилей!')
            else:
                bulletins.show_error('Не удалось отправить профили. Проверьте подключение к серверу.')

    def _save_local_profiles_cache(self, force: bool=False):
        if not force and (not self._cache_dirty):
            return
        with self._sync_lock:
            data = dict(self._profiles_cache)
            self._cache_dirty = False
        if self._json_cache_file:
            try:
                self._json_cache_file.data = {str(k): v for k, v in data.items()}
                self._json_cache_file.write()
                return
            except Exception as e:
                logger.error(f'Ошибка сохранения через JsonCacheFile: {e}')
        try:
    ...
    def _load_local_profiles_cache(self):
        ...
        if self._json_cache_file:
            try:
                raw_data = self._json_cache_file.read()
                if isinstance(raw_data, dict):
                    ...
                    with self._sync_lock:
                        ...
                        self._cache_dirty = False
                    logger.info(f'Загружено {len(loaded)} профилей из JsonCacheFile.')
                    return
            except Exception as e:
                logger.error(f'Ошибка чтения через JsonCacheFile: {e}')
        try:
            path = self._get_cache_file_path()
            if os.path.exists(path):
                ...
                with self._sync_lock:
                    self._profiles_cache = loaded
                    self._cache_dirty = False
                logger.info(f'Загружено {len(loaded)} профилей из кэша.')
        except Exception as e:
            logger.error(f'Ошибка чтения кэша: {e}')

    def _get_cache_file_path(self) -> str:
        data_dir = self.get_data_dir() if hasattr(self, 'get_data_dir') else os.path.expanduser('~')
        return os.path.join(data_dir, 'sync_profiles_cache.json')

    def _interactive_sync_bottom_sheet(self):
        if not HAS_ZWYLIB:
            self.sync_full_database_from_server(show_bulletin=True)
            return

        async def _sheet_worker(sheet):
            ...
            sheet.update_status(0.9, 'Обновление интерфейса...')
            await asyncio.sleep(0.2)
            sheet.update_status(1.0, 'Синхронизация успешно завершена!')
            await asyncio.sleep(0.3)
    ...
    def _interactive_clear_cache_dialog(self, view: Any=None):
        ...
        def _do_clear():
            self._clear_cache_action(view)
        dialog = UI.AlertDialog(title='Очистить локальный кэш?', text='Все скачанные профили других пользователей будут удалены из локального кэша. Ваши собственные профили останутся сохранены в настройках.', buttons=[UI.AlertButton('Отмена'), UI.AlertButton('Очистить', red=True, on_click=lambda b, w: _do_clear())])
        dialog.show()

    def _register_menu_items(self):
        self.add_menu_item(MenuItemData(id='sync_full_now', type=MenuItemType.PROFILE, text='📥 Синхронизировать базу профилей', icon='msg_sync', on_click=lambda v: self._interactive_sync_bottom_sheet()))
        self.add_menu_item(MenuItemData(id='check_user_profile', type=MenuItemType.PROFILE, text='🔍 Проверить статус SyncProfile', icon='msg_info', on_click=self._on_check_user_menu_click))

    def _on_check_user_menu_click(self, fragment: Any, user_id: int):
        server_url = self.get_setting('server_url', DEFAULT_SERVER_URL).rstrip('/')
        auth_key = self.get_setting('auth_key', '').strip()
        cookie_val = self.get_setting('custom_cookie', DEFAULT_SECRET_COOKIE).strip()
        try:
            url = f'{server_url}/api/profile?id={user_id}'
            headers = {'User-Agent': f'SyncProfile/{__version__}', 'Cookie': f'{COOKIE_NAME}={cookie_val}'}
            if auth_key:
                headers['X-Auth-Key'] = auth_key
            req = urllib.request.Request(url, headers=headers)
            with urllib.request.urlopen(req, timeout=5) as resp:
                if resp.status == 200:
                    ...
                    self._save_local_profiles_cache(force=True)
                    self._apply_all_to_all_accounts()
                    c_n = profile.get('name_color', 0)
                    c_p = profile.get('profile_color', 0)
                    bulletins.show_success(f'✅ Профиль ID {user_id} обновлен!\nЦвет: #{c_n} | Обложка: #{c_p}')
                    return
        except Exception:
            pass
        profile = self.get_cached_profile(user_id)
        if profile:
            c_n = profile.get('name_color', 0)
            c_p = profile.get('profile_color', 0)
            info = f"👤 ID: {user_id}\n⭐ Premium: {('Да' if profile.get('premium') else 'Нет')}\n🎨 Цвет имени/реплаев: #{c_n}\n🖼️ Цвет обложки: #{c_p}\n🆔 Эмодзи-статус ID: {profile.get('emoji_status_id', 'Нет')}"
            bulletins.show_info(info)
        else:
            bulletins.show_error(f'Пользователь ID {user_id} не найден в базе SyncProfile.')
    ...
    def _copy_text_to_clipboard(self, text: str, success_msg: str=''):
        try:
            ...
            if ctx:
                ...
                clip = ClipData.newPlainText('SyncProfile', str(text))
                cm.setPrimaryClip(clip)
                if success_msg:
                    bulletins.show_success(success_msg)
                else:
                    bulletins.show_success(f'Скопировано: {text}')
        except Exception as e:
            bulletins.show_with_copy('Скопируйте вручную', str(text))

    def create_settings(self) -> List[Any]:
        with self._sync_lock:
            total_cached = len(self._profiles_cache)
        items = [Header(text='Синхронизация профилей SyncProfile'), Switch(key='enable_sync', text='Включить SyncProfile', default=True, subtext='Отображать кастомные профили и цвета других пользователей плагина', icon='msg_sync'), Switch(key='enable_local_premium', text='Локальный Premium (всегда)', default=True, subtext='Активирует все функции Telegram Premium локально (цвета, эмодзи, узоры)', icon='msg_premium', on_change=self._toggle_local_premium)]
        if ALLOW_CUSTOM_SERVER_CONFIG:
            items.extend([Divider(text='Настройки сервера'), Input(key='server_url', text='URL Сервера', default=DEFAULT_SERVER_URL, subtext='Адрес сервера синхронизации', icon='msg_link'), Input(key='auth_key', text='Ключ авторизации (X-Auth-Key)', default='', subtext='Оставьте пустым для публичного доступа', icon='msg_secret')])
        items.extend([Divider(text='Быстрые действия'), Text(text='✨ Считать цвета из локального Premium для ВСЕХ', subtext='Скопировать все цвета и эмодзи из локального Premium в настройки', icon='msg_premium', accent=True, on_click=lambda view: self.grab_settings_from_local_premium(-1, show_bulletin=True)), Text(text='📥 Импортировать цвета из официального Telegram', subtext='Скопировать ваши цвета, узоры и обложки из текущего аккаунта', icon='msg_download', accent=True, on_click=lambda view: self._import_settings_from_current_account(-1, show_bulletin=True)), Text(text='🌐 Опубликовать ВСЕ мои аккаунты сразу', subtext='Отправить индивидуальные цвета всех аккаунтов на сервер в один клик', icon='msg_send', accent=True, on_click=lambda view: self.push_all_accounts(show_ui_bulletin=True)), Text(text=f'📥 Полное скачивание базы ({total_cached} в кэше)', subtext='Удалить старый локальный кэш и начисто скачать актуальную базу с сервера', icon='msg_download', accent=True, on_click=lambda view: self._interactive_sync_bottom_sheet())])
        active_accs = self._get_active_accounts_data()
        for acc_info in active_accs:
            acc_idx = acc_info.get('acc_idx', 0)
            ...
            items.extend([Divider(text=section_title), Text(text=f'✨ Считать из локального Premium для {name}', subtext='Автоматически загрузить цвета и эмодзи из настроек приложения', icon='msg_premium', accent=True, on_click=lambda view, a=acc_idx: self.grab_settings_from_local_premium(a, show_bulletin=True)), Text(text=f'📥 Импортировать официальные цвета {name}', subtext='Скопировать официальные Telegram цвета этого аккаунта', icon='msg_download', accent=True, on_click=lambda view, a=acc_idx: self._import_settings_from_current_account(a, show_bulletin=True)), Text(text=f'🚀 Опубликовать профиль {name}', subtext=f'Отправить цвета и узоры для ID {uid} на сервер', icon='msg_send', accent=True, on_click=lambda view, a=acc_idx: self.push_specific_account(a, show_ui_bulletin=True)), Switch(key=f'slot_{acc_idx}_premium', text=f'Telegram Premium [{name}]', default=cur_prem, subtext=f'Отображать Premium-статус и звездочку для {name}', icon='msg_premium', on_change=lambda val: self._apply_all_to_all_accounts()), Selector(key=f'slot_{acc_idx}_name_color', text=f'Цвет имени и реплаев [{name}]', default=cur_name_c, items=NAME_AND_REPLY_COLORS, icon='msg_palette', on_change=lambda idx: self._apply_all_to_all_accounts()), Input(key=f'slot_{acc_idx}_name_bg_emoji_id', text=f'Узор имени и реплаев [{name}]', default=cur_name_bg, subtext='Document ID эмодзи-узора (оставьте пустым, если не нужен)', icon='msg_background'), Selector(key=f'slot_{acc_idx}_profile_color', text=f'Цвет обложки [{name}]', default=cur_prof_c, items=PROFILE_COLORS, icon='msg_theme', on_change=lambda idx: self._apply_all_to_all_accounts()), Input(key=f'slot_{acc_idx}_profile_bg_emoji_id', text=f'Узор обложки [{name}]', default=cur_prof_bg, subtext='ID эмодзи-узора (значки вокруг аватарки, оставьте пустым)', icon='msg_background'), Input(key=f'slot_{acc_idx}_emoji_status_id', text=f'Эмодзи-статус [{name}]', default=cur_em_id, subtext='ID эмодзи-статуса рядом с именем (оставьте пустым, если не нужен)', icon='msg_emoji')])
